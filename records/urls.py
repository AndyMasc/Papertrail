...
urlpatterns = [
    ...
    path(
        "add_record/", views.AddRecordView.as_view(), name="add_record_manual"
    ),  # add_record with no document_id. For manual record addition without a document or OCR.
    path(
        "delete_record/<int:pk>/", views.DeleteRecord.as_view(), name="delete_record"
    ),  # handles updating the record, as well as displaying it's details
    path(
